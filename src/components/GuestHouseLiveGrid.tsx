...
import { Clock, Plus, Trash2, Layers, Building2, ChevronDown, ChevronUp, X, Edit2, DoorOpen } from 'lucide-react';
...
export function GuestHouseLiveGrid() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50 flex">
      ...
      {/* Main Content Area */}
      <div className={`flex-1 transition-all duration-300 ${isCollapsed ? 'ml-16' : 'ml-64 sm:ml-80'}`}>
        {/* Header */}
        <div className={`fixed top-0 right-0 z-50 bg-gradient-to-r from-blue-500 to-purple-600 shadow-lg transition-all duration-300 ${isCollapsed ? 'left-16' : 'left-64 sm:left-80'}`}>
          <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-3">
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
              ...
              <div className="flex-1 flex items-center gap-1.5 sm:gap-2 justify-end">
                <Card
                  className={`bg-white/95 backdrop-blur border-0 p-1.5 sm:p-2 cursor-pointer hover:shadow-lg transition-all active:scale-95 flex-1 min-w-0 ${roomFilter === 'all' ? 'ring-2 ring-blue-500' : ''}`}
                  onClick={() => setRoomFilter('all')}
                >
                  <div className="text-center">
                    <p className="text-[10px] sm:text-xs text-gray-600 mb-0.5 sm:mb-1 truncate">{t('dashboard.totalRooms')}</p>
                    <p className="text-base sm:text-xl font-bold text-blue-600">{totalRooms}</p>
                  </div>
                </Card>

                <Card
                  className={`bg-white/95 backdrop-blur border-0 p-1.5 sm:p-2 cursor-pointer hover:shadow-lg transition-all active:scale-95 flex-1 min-w-0 ${roomFilter === 'occupied' ? 'ring-2 ring-green-500' : ''}`}
                  onClick={() => setRoomFilter('occupied')}
                >
                  <div className="text-center">
                    <p className="text-[10px] sm:text-xs text-gray-600 mb-0.5 sm:mb-1 truncate">{t('dashboard.occupied')}</p>
                    <p className="text-base sm:text-xl font-bold text-green-600">{occupiedRooms}</p>
                  </div>
                </Card>

                <Card
                  className={`bg-white/95 backdrop-blur border-0 p-1.5 sm:p-2 cursor-pointer hover:shadow-lg transition-all active:scale-95 flex-1 min-w-0 ${roomFilter === 'vacant' ? 'ring-2 ring-gray-500' : ''}`}
                  onClick={() => setRoomFilter('vacant')}
                >
                  <div className="text-center">
                    <p className="text-[10px] sm:text-xs text-gray-600 mb-0.5 sm:mb-1 truncate">{t('dashboard.vacant')}</p>
                    <p className="text-base sm:text-xl font-bold text-gray-600">{vacantRooms}</p>
                  </div>
                </Card>
              </div>
            </div>
        ...
        {/* Room Grid */}
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6 pt-48 sm:pt-44 md:pt-48 lg:pt-52">
